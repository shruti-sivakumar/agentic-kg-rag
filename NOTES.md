...
Before System C can walk the graph, it needs to know *which entity to start from*. The dataset conveniently already tells us this for each question (it ships the correct starting entity IDs, the same way it ships the fact-chain), and this step uses that directly rather than working it out. That's worth being honest about as a real, deliberate shortcut for this phase, not something to wave away — a genuine entity-linking system (fuzzy name matching, disambiguating between candidates) is harder and is exactly what the later MuSiQue/HotpotQA extension has to build, since those datasets don't ship gold starting IDs the way this one does.

The reason this shortcut doesn't quietly undermine the whole result is a specific structural fact, not just a general excuse: the starting entity is always a name that's *sitting directly in the question text* ("Princess Alexandrine of Prussia," not some hidden intermediate person the system would have to discover). Identifying it is a different problem from the multi-hop reasoning being tested — it doesn't leak any part of the fact-chain, which is the thing that would actually matter as cheating. And unlike the entity-linking shortcut, *that* claim isn't just argued — it's measured: every single hop after the starting point (step 8's graph-walk) gets no such help and has to pick the right fact among every real connected option with no ground-truth signal, and it does that correctly 87.7% of the time on the questions where there was a genuine choice to make. That's the real evidence this isn't the graph secretly reading the answer key — not the entity-ID shortcut being explained away, but the harder part being checked and holding up.

This isn't just a convenient excuse specific to this project, either — it's checked directly against real published work in the field, not just assumed: papers doing this kind of graph-walking question answering (MetaQA, AAAI 2018; DoG, 2024, which evaluates on this exact same 2WikiMultiHopQA dataset; ProgRAG, 2025) all start from a *given* entity the same way, and reserve any gold reasoning-path data for evaluation only, never for the walk itself. This project follows that same convention, not an invented one.
