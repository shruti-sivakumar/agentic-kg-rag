...
**On System C's entity-ID shortcut and why it doesn't undermine the oracle-trap safeguard:** System C's entity linker (`agents/nodes/entity_linker.py`) resolves each question's starting entity using the dataset's own gold Wikidata ID rather than performing real disambiguation — a documented simplification for this phase, not a solved problem being glossed over, and it does not extend to the planned MuSiQue/HotpotQA work, which ships no such ID and needs genuine fuzzy-match/encoder-based linking. This shortcut is defensible specifically because the starting entity is always a name already present in the question text, never a hidden intermediate entity the walk would need to have discovered — resolving it is a different problem from the multi-hop reasoning under test, and leaks no part of the reasoning path. The claim that the reasoning path itself isn't leaked rests on a separate, measured result, not on that argument alone: every hop *after* the starting entity is selected by `graph_traverse.py`'s edge scoring with no gold signal at all, and was checked directly against 138 questions with a genuine choice among real candidates — correct 87.7% of the time. That number, not the entity-ID shortcut, is the actual evidence against the oracle trap.

Taking the starting entity as given, rather than resolved by the system, is also the prevailing convention in the KGQA literature: MetaQA (Zhang et al., *Variational Reasoning for Question Answering with Knowledge Graph*, AAAI 2018) defines each question as a given head entity plus a reasoning path plus an answer. DoG (Li et al., *Decoding on Graphs: Faithful and Sound Reasoning on Knowledge Graphs*, 2024) evaluates on WebQSP, CWQ, and this project's own dataset, 2WikiMultiHopQA — initializing its subgraph from a given query entity, and using ground-truth reasoning paths for evaluation only, available for 2WikiMultiHopQA but not WebQSP/CWQ in its setup, the same role gold paths play in this project. ProgRAG (Park et al., *ProgRAG: Hallucination-Resistant Progressive Retrieval and Reasoning over Knowledge Graphs*, 2025) likewise begins each question's retrieval from a given source entity.
